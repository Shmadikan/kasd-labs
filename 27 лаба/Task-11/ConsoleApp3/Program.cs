...
namespace ConsoleApp3
{
    ...
    internal class Program
    {
        ...
        public class MyPriorityQueue<T>:MyQueue<T> where T : IComparable
        {
            protected MyVector<T> queue;
            ...
            public class MyItr : MyIterator<T>
            {
                ...
                MyPriorityQueue<T> includedQueue;
                MyPriorityQueue<T> CopyOfOriginal;

                internal MyItr(MyPriorityQueue<T> Squeue) {
                    
                    T[] array = Squeue.ToArray();
                    CopyOfOriginal = Squeue;
                    includedQueue = new MyPriorityQueue<T>(array);
                    Insize = Squeue.size;
                }
                ...
                public T Next()
                {
                    T el = includedQueue.Pool();
                    current = el;
                    Insize = includedQueue.Size();
                    return el;
                }
        ...
        static void Main() {
            int[] mas = { 4, 3, 1,7 , 5, 9, 8, 16, 15};
            char[] mas2 = { 'a', 'b', 'c' };

            MyPriorityQueue<int> queue = new MyPriorityQueue<int>(mas);
            var el = queue.Iterator();
            while (el.HasNext()) {
                el.Next();
                Console.WriteLine(el.Current);
                el.Remove();
            }
