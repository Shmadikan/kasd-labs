...
namespace ConsoleApp1
{
        public class MyHashMap<K, T>:MyMap<K, T> where T:IComparable {
            ...
            public T Get(object key) {
                K CKey = (K)key;
                var el = Entry[Math.Abs((CKey.GetHashCode()) % Entry.Length)];

                for (int i = 0; i < el.Size(); i++)
                {
                    Tuple<K, T> tup = el.Get(i);
                    if (tup.Item1.Equals(CKey))
                        return tup.Item2;
                }
                return default(T);
            }
            ...
            public MyCollection<T> Values() { 
                ...
                for (int i = 0; i < Entry.Length; i++) {
                    if (Entry[i] != null) {
                        for (int j = 0; j < Entry[i].Size(); i++) {
                            linkedList.Add(Entry[i].Get(j).Item2);
