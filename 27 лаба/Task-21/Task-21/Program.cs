...
namespace Task_21
{
    ...
    public class MyTreeMap<K, T>:MySortedMap<K, T> where K: IComparable where T:IComparable {
        IComparer comparer;
        ...
        public MyCollection<T> Values() { 
            ...
            foreach (var el in BFS(root.Key)) {
                myArrayList.Add(el.Item2);
        ...
        public MyTreeMap(MyMap<K, T> map)
        {
            comparer = new MyComparator<K>();
            MyHashMap<K, T> CloneMap = (MyHashMap<K, T>)map;
            var Tuples = CloneMap.EntrySet();
            foreach (Tuple<K, T> i in Tuples)
            {
                Put(i.Item1, i.Item2);
            }
        }

        public MyTreeMap(MySortedMap<K, T> map) {
            comparer = new MyComparator<K>();
            foreach (var el in map.EntrySet()) {
                Put(el.Item1, el.Item2);
            }
        }

        public void PutAll(MyMap<K, T> map) {
            MyHashMap<K, T> CloneMap = (MyHashMap<K, T>)map;
            var Tuples = CloneMap.EntrySet();
            foreach (Tuple<K, T> i in Tuples) {
                Put(i.Item1, i.Item2);
        ...
        public Tuple<K, T> PollLastEntry() {
            ...
            while (copy.right != null)
                copy = copy.right;
            Remove(copy.Key);
            return new Tuple<K, T>(copy.Key, copy.Value);
        }
        ...
        public MySortedMap<K, T> HeadMap(K end) {
        ...
        public MySortedMap<K, T> SubMap(K start, K end) {
        ...
        public MySortedMap<K, T> TailMap(K start) { 
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            myTreeMap.Put(2, 11);
            myTreeMap.Put(1, 12);
            MyArrayList<int> sss = new MyArrayList<int>();
