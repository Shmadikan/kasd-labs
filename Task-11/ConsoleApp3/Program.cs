...
namespace ConsoleApp3
{
    ...
    {
        public interface CompPriorityQueue<T>: IComparable<T>, IComparer<T> { 
        ...
        public class MyComparer<T> :IComparer<T> where T:IComparable<T>
        {
        ...
        public class MyPriorityQueue<T> : MyComparer<T> where T:IComparable<T>
        {
        ...
        {
            int[] mas2 = { 1, 2, 3, 4 };
            double[] mas = {2.1, 2.2, 2.3};
            char[] arr = { 'a', 'b', 'c' };
            string?[] sss= { "a", "b", "c" };

            MyComparer2<string?> s = new MyComparer2<string?>();
            MyPriorityQueue<string> Pqueue = new MyPriorityQueue<string>(sss);
            Pqueue.queue.Print();
            Console.WriteLine("");
            while (Pqueue.Size() > 0)
            {
